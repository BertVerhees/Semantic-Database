package nl.rosa.semanticdatabase.archetypeobjectmodel.adlparser.treewalkers;
    ...
    private Expression parseEqualityExpression(EqualityExpressionContext context) {
        if(context.equalityBinop() != null) {
            ...
            if(left.getType() != null && right.getType() != null && left.getType() != right.getType()) {
                throw new IllegalArgumentException("arithmetic relop expression with different nl.rosa.semanticdatabase.bmm.model.types: " + left.getType() + " + " + right.getType());
            }
