 ...
 * This division is reflected in the two top-level meta-classes BMM_CLASS and BMM_TYPE, and their respective descendants, as shown in the following UML diagram.
 *Classes are definitional entities, while types are understood in BMM as the formal generators of instances (where non-abstract), as the basis of static typing in a formal model, and dynamic conformance, i.
 *e.
 ...
 * Types are used for the following purposes in a BMM model:A simple example of typing of properties is shown in the instance diagram below.
 *Most types are based on one or more defining class(es), which provide the formal definition for the type.
 * The exceptions are formal generic parameters (e.
 *g.
 ...
 * The taxonomy is illustrated below, including differentiae.
 *BMM_TYPE includes features common to all meta-types:Below BMM_TYPE are the abstract meta-type BMM_UNITARY_TYPE and the concrete meta-type BMM_CONTAINER_TYPE and its specialisation BMM_INDEXED_CONTAINER_TYPE.
 * BMM_UNITARY_TYPE is a meta-type for types whose instances are unitary i.
 *e.
 * singular, while the container meta-types correspond to collections of instances.
 * The latter are further described below.
 * This distinction is made to enable BMM to directly represent the notion of collections in the type system rather than treating them in the same way as any other type, which would force modellers (i.
 *e.
 * authors of actual BMM models) to state concrete containment types such as ArrayedList<Packet>, where ArrayedList would have to be defined in the model as well as Packet.
 *Unitary meta-types are further distinguished as formal generic parameters (BMM_PARAMETER TYPE) and effective types, i.
 *e.
 * concrete unitary types.
 * The former meta-type is used to represent replaceable formal generic parameters (typically 'T', 'U' within types such as List<T> etc) within generic type declarations.
 *Effective types have as their meta-type BMM_EFFECTIVE_TYPE.
 ...
 *BMM_MODEL_TYPE divides into BMM_SIMPLE_TYPE and BMM_GENERIC_TYPE, corresponding to the standard notions of types familiar in modern programming languages.
 * The class definitions of instances (i.
 *e.
 * BMM model class definitions) of these meta-types are available via the property base_class, of meta-type BMM_CLASS for a BMM simple type, and BMM_GENERIC_CLASS for BMM generic type.
 ...
 *1.
 * Conformance
 * Conformance of Simple types follows the inheritance hierarchy via which they are defined, according to the rules: meta-rule: A Simple type can only conform to a Model type, i.
 *e.
 ...
 *generic_parameters) for any formal parameter (BMM_GENERIC_CLASS.
 *generic_parameters) is of meta-type BMM_UNITARY_TYPE.
 *A typical programmatic usage of such a type, and its instantiated BMM model structure is shown below.
 *The parameters of a generic type may be:Consequently, a generic type may be:The first case is detected via the function is_closed defined on BMM_GENERIC_TYPE, while the function is_partially_closed distinguishes the latter two cases.
 *The following shows the BMM instance structure of a generic type that is open.
 *Since the meta-type of BMM_GENERIC_TYPE.
 *generic_parameters in the BMM is BMM_UNITARY_TYPE, constructions such as MyGenericType<List<OtherType>> are prevented, due to the concrete parameter type List<OtherType> being an instance of BMM_CONTAINER_TYPE rather than of BMM_UNITARY_TYPE.
 ...
 * The following diagram shows the BMM concrete model structure created for a generic type whose formal parameter type is used in this way, i.
 *e.
 ...
 *4.
 * Tuple Meta-type
 * The meta-type BMM_TUPLE_TYPE enables the type of a tuple i.
 *e.
 * an array of objects each of which may be of any type, to be stated in terms of other types.
 * This is mostly used to state the type of a set of arguments in the BMM_SIGNATURE class, but standalone tuple types may also be used in a model, providing roughly the effect of an anonymous class (or a struct in C/C++).
 *The notional type Tuple defined by BMM_TUPLE_TYPE is treadted as a BMM built-in, and is therefore not stated in any BMM model.
 *
 * 
 * 
 * 5.
 ...
 * The simplest type of signature is that of properties and constants, which may be expressed formally in a typical abstract syntax as follows:This indicates that both have a return type, being the type of the value they are attached to at execution time.
 *The general form of routine signatures is as follows:In the above, the construction [Type, …​] represents a type-tuple.
 *Typical examples of function signatures are as follows:Typical procedure signatures include the following:In order to support functional semantics such as function-as-object, the notion of signature must exist as a first order type.
 * Since signatures have a special structure and usage, they have their own meta-type in BMM, BMM_SIGNATURE.
 * Instances of this meta-type represent any kind of signature, and consist of argument_types, whose meta-type is BMM_TUPLE_TYPE (See [Tuple Type]), and result_type, representing the result type.
 *The following signatures state the types 'any function' and 'any procedure':As for the Tuple meta-type, the notional Signature meta-type is a BMM built-in, and is not itself defined in any BMM model.
 *
 * 
 * 
 * 
 ...
 *.
 *.
 *], T_result> -- Function type <[T_arg1, T_arg2, .
 *.
 *.
 *]> -- Procedure type
 * 
 ...
 * BMM_GENERIC_TYPE is thus used for objects considered to be singular, but whose types are a product of the base class and one or more parameter types, e.
 *g.
 * Interval<Quantity>.
 *The explicit provision of BMM_CONTAINER_TYPE enables BMM models to mention logical linear container types such as List<T> and Set<T>, on the assumption of their standard semantics in computer science , without worrying about providing concrete types which may be numerous and also variable across programming languages, e.
 *g.
 * ArrayedList<T>, LinkedSet<T>, ArrayedStack<T> and so on.
 *List and Set semantics are achieved via the BMM_CONTAINER_TYPE attributes is_ordered and is_unique, used in the standard combinations i.
 *e.
 ...
 *The semantics of indexed containers, commonly known under the type names Hash<K,V>, HashMap<K,V>, HashTable<K,V>, Dictionary<K,V> and so on, are represented by the meta-type BMM_INDEXED_CONTAINER_TYPE, which inherits from BMM_CONTAINER_TYPE, and adds the property index_type.
 * The latter type represents the key type, which must be such that hash values can be generated, and may be any type, but practically speaking, is almost always a String, Integer, or a Date/Time type.
 ...
 *7.
 * Type Conformance
 * An algorithm to determine conformance of two type-names (e.
 *g.
 * to implement BMM_MODEL.
 *type_conforms_to()) is as follows:
 * 
 * 
 * 
 * Boolean type_conforms_to (String a_type, anc_type) { BMM_TYPE_NAME a_type_name, anc_type_name; if attached create_type_name_from_string (a_a_type) as a_type_name and attached create_type_name_from_string (anc_type) as anc_type_name { base_class = a_type_name.
 *name; anc_base_class = anc_type_name.
 *name; if (base_class.
 *is_case_insensitive_equal (anc_base_class) or else class_definition (base_class).
 *has_ancestor_class (anc_base_class)) { BMM_CLASS bmm_def_class = class_definition (base_class); // ---- Tuple types // ---- Signature types ---- // ---- Generic types // handle case where formal generic names appear in type name if (valid_generic_type_name (a_type) and bmm_def_class instanceOf (BMM_GENERIC_CLASS) { // in the case of both being generic, we need to compare generics // to start with, the number of generics must match BMM_CLASS anc_bmm_def_class = class_definition (anc_base_class); if (valid_generic_type_name (anc_type) and anc_bmm_def_class instanceOf (BMM_GENERIC_CLASS)) { ArrayedList<String> a_type_gen_params = a_type_name.
 *generic_parameters_type_list; ArrayedList<String> anc_type_gen_params = anc_type_name.
 *generic_parameters_type_list; if (a_type_gen_params.
 *count = anc_type_gen_params.
 *count) { Iterator<String> this_gen_parms_it = a_type_gen_params.
 *iterator(); Iterator<String> other_gen_parms_it = anc_type_gen_params.
 *iterator(); Boolean result = True; String a_type_gen_type, anc_type_gen_type; while (this_gen_parms_it.
 *hasNext() && other_gen_parms_it.
 *hasNext() || !result) { // first we convert any open generic parameters to their conformance types // We assume type names of 1 letter are open parameters String this_gen_parm = this_gen_parms_it.
 *next(); String other_gen_parm = other_gen_parms_it.
 *next(); if (formal_generic_parameter_name (this_gen_parm)) a_type_gen_type = this_bmm_gen_class.
 *generic_parameter_conformance_type (this_gen_parm); else a_type_gen_type = this_gen_parm; if (formal_generic_parameter_name (other_gen_parm)) anc_type_gen_type = other_bmm_gen_class.
 *generic_parameter_conformance_type (other_gen_parm); else anc_type_gen_type = other_gen_parm; -- now do the test result = type_conforms_to (a_type_gen_type, anc_type_gen_type); } return result; } } // Conforms - case where anc type is not provided in generic form, but desc is // e.
 *g.
