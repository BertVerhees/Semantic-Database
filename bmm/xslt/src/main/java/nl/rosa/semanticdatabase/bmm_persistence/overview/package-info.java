 ...
 * Instead of directly materialising a BMM instance graph from serial form, the initial materialised form is a graph of P_BMM_XXX classes, with a subsequent in-memory model-to-model transform step required to produce a materialised BMM model.
 *The P_BMM_* classes perform two functions.
 ...
 * This enables an object model represented internally (to a tool, say) in BMM form, converted to P_BMM form, and then serialised to a .
 *bmm file, to be easily readable and editable by human users.
 *The second is that .
 *bmm files function as schemas that support schema inclusion and therefore re-use, in a similar manner to the XML schema languages.
 * Thus, a single logical BMM model can be expressed as a number of .
 *bmm schema files which are actually P_BMM_* object serialisations of parts of the BMM model.
 * A schema reading component has to resolve the schema inclusions and ultimately BMM_* object instantiations to obtain the in-memory form of the model.
 ...
 *The P_BMM format is not the only serial format possible for BMM, and alternatives, e.
 *g.
 * a more syntactic style reminiscent of OMG IDL, are possible.
