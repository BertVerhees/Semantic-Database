 ...
 * The latter is what programmers understand as a 'call', i.
 *e.
 * an application of a lambda term.
 *An agent is formed by reference to a function or procedure in an expression context, potentially with provision of some or all arguments.
 ...
 * A procedure call has no evaluation type.
 *Both calls and agents are special kinds of references to their original routines, and both can appear within an expression context, including as arguments passed to routine invocations.
 * The following UML view of the bmm.
 *expression package illustrates these details.
 *
 * 
 * 
 * 
 ...
 * Figure 38.
 * bmm.
 *expression package - Functional Elements
 * 10.
 *2.
 * Agents
 ...
 * This includes other agent expressions, and also function calls (but not procedure calls, since they are not value-generating entities).
 *An example of a delayed function call in syntax form is nodeCount (struct=?, depth=3) (the exact syntax will vary according to programming language).
 ...
 * For such a call to be possible, all arguments must be supplied (which may be none, in the case of a parameterless function).
 *An example of a Function Call is calculateAge ('1982-03-22', '2019-06-01'), which calculates a person’s age from his date of birth and a given date.
 * In normal programming languages, any of the parameters may be any referenceable value-returning entity, i.
 *e.
 * any in-scope variable, other function call, or a valid operator expression, e.
 *g.
 * calculateAge ('1982-03-22', today()), where today() is a function returning the date of today.
 *Such calls are direct equivalents for expressions using basic mathematical operators and functions, on the usual basis that any operator (such as 'plus', i.
 *e.
 * +) is formally defined as a function such as add (arg1, arg2: Numeric): Numeric.
 *The instance structure for a typical expression such as the Body Mass Index (BMI), i.
 *e.
