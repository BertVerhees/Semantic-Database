 ...
 * This is particularly true of openEHR and other archetype-based frameworks, where a further need is to be able to validate archetypes and templates with respect to the reference model, and also to validate runtime instance data against operational templates and the reference model.
 *A number of computable representations of the openEHR published models have been available in the past.
 * Currently models are represented in two computable forms, namely UML and BMM (i.
 *e.
 * the format described in this specification).
 ...
 * It can also be serialised and used computably, e.
 *g.
 ...
 * XMI is also notoriously non-standard across UML tools.
 ...
 *The BMM provides a standalone alternative to UML/XMI which correctly represents all types, including open and closed generic types, inheritance of generic types, and various other problems with UML.
 * As a meta-model it is adapted to the task, i.
 *e.
 ...
 *2.
 * Features
 * The Basic Meta-Model supports the representation of object models in the ISO RM/ODP information and computational points of view, i.
 *e.
 * in programming terms:It is designed to enable both human authoring and machine processing, including e.
 *g.
 * extraction of BMM textual schemas from a UML tool or programming language classes.
 * The semantics of the model are heavily influenced by the formal approach to object-orientation described by Bertrand Meyer in Object-oriented Software Construction (Meyer, 1997) and also the Eiffel language, which is significantly better basis for object modelling than the UML 2.
 *x meta-model.
 * The BMM consequently departs from UML in a number of significant ways, and also from the OOSC/Eiffel approach in some aspects (e.
 *g.
 * direct meta-type suport for container types).
 * Its key features are as follows:Functional elements are supported via the inclusion of meta-types representing signatures and tuples, enabling the construction of delayed agent calls, known here as agents, and function applications (i.
 *e.
 * function calls).
 *Note that there is no BMM entity for graphical diagramming semantics.
 ...
 * UML 2.
 *x and its associated serialisation format XMI 2.
 *x should in theory mean that complete, interoperable machine expressions of object models would be available in all tools.
 * However the evolution of UML and XMI has not been toward a clear meta-model and language for each of its sub-languages (i.
 *e.
 ...
 * The UML 2.
 *x specifications are exceedingly complex: UML 2.
 *1.
 *2 was specified by two documents, 'Superstructure' (738pp) and 'Infrastructure' (224pp); the UML 2.
 *5.
 *1 specification is only slightly smaller at 796 pages (see the OMG UML page for current specifications).
 * The XMI 2.
 *x specification is correspondingly complex, which seems to have so far prevented reliable tool interoperability (recognised as a critical issue by OMG in 2015).
 * Despite the complexity, there are significant limitations in the UML meta-model, including: the composite or association nature of a class property only being known if it is represented as an association, i.
 *e.
 * a line linking two classes on a UML diagram; the notion of type is not adequately formalised; the meta-model of generic types and container properties is problematic and does not map well to object programming languages; the Design By Contract (DbC) concept (i.
 *e.
 * pre-, post-conditions, class invariants), crucial for proper specifications, is in theory supported via the use of OCL 2.
 *0 constraints in class definitions, but OCL suffers from the same underlying semantic weaknesses as UML; there is no direct support for functional entities, i.
 *e.
 * routine calls as objects (aka 'lambdas').
 * Experience with various UML tools (up till 2015) highlighted the following problems: poor support for OCL and design by contract in most tools; variable support for generic (i.
 *e.
 * template) classes; even those tools that properly implement the UML 2.
 *5 specification are still very hard to use for defining generic classes because of problems in the specification that remain unaddressed; problems with qualified attributes, which are used to represent identifier references to objects rather than direct references; variable support for XSD generation across tools, where the results are wildly wrong in some tools; in some tools, it is impossible to define an abstract formal model - the only option is to select a particular programming language profile such as Java or C# and thus get locked into the limitations of those languages (messy type systems, weak inheritance semantics, language-specific notion of types such as Array<T>, List<T>, etc.
 *).
 ...
 * Nevertheless, a small number of tools, including MagicDraw (currently used for representing openEHR models for the specifications) and Rational Software Architect (RSA), appear to implement UML 2.
 *5 faithfully.
 * This means that despite the limitations of UML 2.
 *5 (as noted above), models expressed in it can be mostly correctly interpreted and corrected by post-processing for purposes such as code generation and specifications publishing.
 ...
 * The BMM from version 3.
 *0.
 *0 on may be used to represent a full class model at an interface level (i.
 *e.
 * without code for methods) including classes, types, and class feature types including property (i.
 *e.
 ...
 * Until version 3.
 *0.
 *0, BMM supported only the information point of view, i.
 *e.
 ...
 * This specification defines a BMM object model, i.
 *e.
 ...
 * The latter enables serialisation of a BMM into a concrete syntax such as ODIN, JSON or XML.
 *The BMM packages are as follows:Related packages are:These are illustrated below.
 *
 * 
 * 
 * 
