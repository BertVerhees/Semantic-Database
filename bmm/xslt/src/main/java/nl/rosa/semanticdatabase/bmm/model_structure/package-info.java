 ...
 * A BMM model is structured in the same general way as a UML model, i.
 *e.
 ...
 * The following illustrates the structure of a typical 'class' or 'object' model.
 *The BMM_MODEL class defines the single instance of each distinct BMM model that may exist within a collection of models, such as shown in the model_access package above.
 ...
 * Package paths are only used in BMM to specify package structures in the serialised form in an efficient way, i.
 *e.
 ...
 * In BMM, the use of other models is declared once only, in the using model, rather than on a per-class level as in some languages (e.
 *g.
 * Java).
 * The use of another model makes its contents available for use in declarations (e.
 *g.
 * inheritance) and expressions within the using model.
 * This is achieved by populating the BMM_CLASS.
 *scope attribute with a reference to the used model.
 ...
 * Similar to the root class in a typical OOPL type systems (sometimes called 'Object'), the Any class defines semantics true for all objects such as equality (i.
 *e.
 * semantics for an '=' operator) and copying.
 *A BMM model may define its own Any class, but if it does not, the BMM_MODEL instance representing the model will produce a standard 'Any' class via the any_class_definition() method.
 * This will create the following structure, including a default package structure, and an Any type.
 *The Any type defined by the model’s Any class, or else the default one above, will be used as the inheritance parent for every class in the model that doesn’t have any other inheritance parent.
