 ...
 * The overall meta-model of features, as well as the elements that occur within them, namely parameters and local variables is shown below, and described in this section.
 ...
 * Feature groups may be given any name, and are usually used to group features on the basis of things like: meta-type, e.
 *g.
 * a group called "rules" limited to publicly visible functions only; logical feature type, e.
 *g.
 ...
 *2.
 * Feature Visibility
 * Selective visibility of features to external clients is expressed in the attributes BMM_FEATURE_GROUP.
 *visibility with potential overrides achieved using BMM_CLASS_FEATURE.
 *visibility, i.
 *e.
 ...
 * Those elements defined within a class scope have a type signature, i.
 *e.
 ...
 * Features, which are declarations in context can be mutable, but value objects (e.
 *g.
 ...
 * The BMM_TYPED hierarchy is the one requiring type substitutability, and is thus implemented using class inheritance (e.
 *g.
 * extends in Java), while the other should be implemented by inheritance of interfaces.
 * This can be achieved as follows: define the BMM classes BMM_ROUTINE, BMM_CLASS_FEATURE, BMM_CLASS_ENTITY, BMM_DECLARATION etc as interfaces, with the attributes (such as BMM_ROUTINE.
 *parameters) declared as methods and declare the inheritance as interface inheritance (e.
 *g.
 * implements in Java); define a parallel class hierarchy BMM_ROUTINE_IMPL, etc whose design follows the UML model above (i.
 *e.
 * defines real attributes) and which are declared as the types of private data elements in the main classes (e.
 *g.
 ...
 *7.
 * Signatures
 * All class-scoped features have formal signatures (described in Section 5.
 *5), realised as BMM_CLASS_ENTITY.
 *signature.
 * This includes procedures, even though they have no return type.
 * The two types of routines, i.
 *e.
 ...
 * The standard taxonomy of feature signature types is illustrated below, with each form being controlled by invariants stated in the relevant containing meta-classes (e.
 *g.
 * BMM_FUNCTION, representing a function definition has an invariant requiring signature.
 *result /= Void).
 * Figure 28.
 * Signature taxonomy
 * 7.
 ...
 * An example of the use of this is described below in Section 12.
 *1.
 *2.
 *
 * 7.
 *2.
 * Constants
 * Constant features in a class are represented by the BMM meta-type BMM_CONSTANT, which is:At runtime a constant has a value, which may be of any type, including complex types, and which is set using its generator expression (i.
 *e.
 ...
 * This latter capability is the means by which computed constants are supported.
 ...
 * 
 * 
 * class BASIC_DEFINITIONS { feature // // simple constants // constant CR: Character = '\015'; constant LF: Character = '\012'; // // constant of complex type // TODO: correct JSON (maybe YAML?) or other syntax // constant SafeSpeed: Interval<Quantity> = |0.
 *0 km/h .
 *.
 * 60.
 *0 km/h|; // // constant generated by a one-time expression evaluation // constant ProxyVar<Quantity> = {ProxyVar<Quantity>}(arg1, arg2); }
 * 7.
 *3.
 * Properties
 ...
 * The first two properties are associations, used to represent references to shared objects (including singletons), while the remainder are normal compositional sub-parts, i.
 *e.
 * owned sub-instances that will cease to exist when the parent object is destroyed.
 * All of the properties are single-valued, other than other_participations, which is defined as a container property with cardinality [*] (i.
 *e.
 * 0.
 *.
 **) and 'set' semantics.
 * abstract class ENTRY extend CARE_ENTRY { feature -- Locale association property language: CODE_PHRASE <<"iso::iso_639-2">> [0.
 *.
 *1]; association property encoding: CODE_PHRASE <<"iana::character-sets">> [0.
 *.
 *1]; feature -- Access composition property protocol: ITEM_STRUCTURE[1]; composition property guideline_id: OBJECT_REF[0.
 *.
 *1]; composition property subject: PARTY_PROXY[0.
 *.
 *1]; composition property other_participations: PARTICIPATION[*]{set}; }
 * 7.
 *3.
 *2.
 * Semantic level
 ...
 * The classes BMM_FUNCTION, BMM_PROCEDURE and BMM_ROUTINE respectively provide the meta-model for definitions of the two types of routine.
 *The formal parameter definitions of both kinds of routines are represented by an ordered list of instances of the meta-class BMM_PARAMETER, which is defined as a kind of variable.
 * A formal parameter may optionally have the read/write direction defined, which indicates whether the variable contains an input value, should only be written to, or both, i.
 *e.
 * read then written to.
 * The is_nullable meta-attribute inherited from BMM_TYPED_FEATURE indicates whether each parameter is mandatory or not.
 *An additional feature of the meta-model for functions is the notion of operator, which supports the representation of a function call in a formal expression as an operator symbol with one or two operands, in the typical form found in programming languages and first order logics.
 ...
 * Operators thus support the use of expressions like weight > 90 to mean weight.
 *gt (90), where weight is of type Real, and the function gt(other: Real) is defined.
 *Any routine may have pre- and/or post-conditions defined for it, represented in BMM by BMM_ASSERTIONs populating the pre_conditions and post_conditions attributes of BMM_ROUINE.
 * An Assertion is a tagged Boolean Expression.
 ...
 * The essential semantics are as follows: pre-conditions specify truth conditions on public object state and/or parameters that must hold for the routine to execute correctly; if a pre-condition is not met at the moment of routine call, the client (caller) is at fault; post-conditions specify truth conditions on public object state and/or result (in the case of functions) that will hold at the end of routine execution; if a post-condition is not met at the moment of routine completion, the routine is at fault, i.
 *e.
 * contains bugs.
 ...
 * class I_EHR_SERVICE { feature -- Creation create_ehr_with_id ( an_ehr_id: UUID[1]; an_ehr_status: EHR_STATUS[0.
 *.
 *1] ): UUID[1] pre_condition No_duplicate: not has_ehr (an_ehr_id) post_condition Has_ehr: has_ehr (Result) }
 * 7.
 *4.
 *2.
 * Creators and Converters
 * In common with may OOPLs, BMM classes may specify creators, i.
 *e.
 * instance-creating procedures, known as 'constructors' in some OOPLs.
 * These are just a subset of all procedures known in BMM_CLASS.
 *procedures that when executed will satisfy the class invariants.
 ...
 * Converter routines are typically used to support type conversion in expressions, e.
 *g.
 * String to Date etc.
 ...
 *3.
 * Routine Body
 * BMM supports the definition of statements, i.
 *e.
 ...
 * The major disadvantage of the latter is that it makes procedure definitions (BMM_PROCEDURE instances within a BMM model) look like typed entities, and as a result, procedure calls look like value-returning entities that could be used within expressions (i.
 *e.
 * no different than function calls).
 *Ultimately this difficulty arises from the decision to include procedures at all within the formal framework, when from a purely functional perspective they would not exist.
 ...
 * BMM has taken the route of objects as black boxes having queries (functions, properties) and commands (procedures), with well-behaved models relying on the principle of command-query separation (CQS), i.
 *e.
