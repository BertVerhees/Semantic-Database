 ...
 * This facility is not needed for achieving the original purpose of BMM, i.
 *e.
 * a computable in-memory representation of a model at the interface level (classes, types and signatures), rather it provides a basis for representing program logic, such as function definitions.
 ...
 *The use of the three-class hierarchy pattern allows a Routine body to be a simple statement or a block, containing statements and/or more blocks.
 ...
 * A number of common types are included by default, and are described below.
 ...
 * Type checking is thus based on a comparison of definition.
 *type of the target and eval_type() of the source.
 ...
 * 
 * 
 * class Simple_bank_account { feature // Definitions overdraft_limit: Decimal = 250; feature // State balance: Decimal; feature // Interface position: Decimal { Result := balance + overdraft_limit } invariant Solvent: position >= 0.
 *0 }
 * 
 * 
 * 
 ...
 * 
 * 
 * class Simple_bank_account { feature // Modification put (val: Decimal) precondition valid_amount: val + overdraft_limit >= 0.
 *0 { balance := balance + val } } class Account_access { feature // Command deposit (v: Decimal; ac: Simple_bank_account) pre_condition valid_amount: v > 0.
 *0 { ac.
 *put (v); } withdraw (v: Decimal; ac: Simple_bank_account) pre_condition valid_amount: ac.
 *overdraft_limit - v >= 0.
 *0 { ac.
 *put (-v); } }
 * 
 ...
 * An action table corresponds closely to the standard constructs 'if/then/elseif' and the case statement.
 ...
 * 
 * 
 * check_vital_signs if systolic_pressure.
 *in_range ([critical_high]) raise_alarm ([emergency]) elseif systolic_pressure.
 *in_range ([high]) and oxygen_sat.
 *in_range([low]) call_staff
 * 11.
 *5.
 * Assertions
