 ...
 *bmm.
 *core.
 *literal_value package - Literal Values
 * 8.
 *2.
 * General Model
 * When a BMM model is instantiated in its in-memory form, e.
 *g.
 * in a BMM modelling tool, any literal values in the model such as constant values (e.
 *g.
 * Real Pi = 3.
 *1415926), default values (e.
 *g.
 * Person.
 *name: String = "(unknown)") etc are instances of 'two types':The value of any such instance is represented by the value_literal field within its BMM_LITERAL_VALUE.
 ...
 * This may be a programming language syntax, JSON, or any other serialisable format, and is specified by BMM_LITERAL_VALUE.
 *syntax.
 *
 * 
 * 
 ...
 * Similarly a syntax that supported literal Sets using braces, e.
 *g.
 * {"Feyerabend", "Lakatos", "Popper"} may be parsed to a Set<String> instance, with the same meta-type as for the List.
 ...
 * Tuple instances are used in BMM expressions and described in Section 9.
 *2.
 *
 * 
 * 
